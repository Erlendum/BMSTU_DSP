...
main <- function() {
  ...
  lines(t, x2, col = "blue")
  legend("topleft", legend = c("No noise", "Gaussian noise", "Impulsive noise"), col = c("red", "green", "blue"), lty = 1, cex = 0.6)
  
  # Gaussian filter (high freq)
  plot(t, x0, col = "red", type = "l", main = "Gaussian filter (high freq)", ylab = "Amplitude", xlab = "Time")
  lines(t, filter(x1, GH, circular = TRUE), col = "green")
  lines(t, filter(x2, GH, circular = TRUE), col = "blue")
  legend("topleft", legend = c("No noise", "Gaussian noise", "Impulsive noise"), col = c("red", "green", "blue"), lty = 1, cex = 0.6)
  
  # Gaussian filter (low freq)
  plot(t, x0, col = "red", type = "l", main = "Gaussian filter (low freq)", ylab = "Amplitude", xlab = "Time")
  lines(t, filter(x1, GL, circular = TRUE), col = "green")
  lines(t, filter(x2, GL, circular = TRUE), col = "blue")
  legend("topleft", legend = c("No noise", "Gaussian noise", "Impulsive noise"), col = c("red", "green", "blue"), lty = 1, cex = 0.6)
  
  # Butterworth filter (high freq)
  plot(t, x0, col = "red", type = "l", main = "Butterworth filter (high freq)", ylab = "Amplitude", xlab = "Time")
  lines(t, filter(x1, BBH, circular = TRUE), col = "green")
  lines(t, filter(x2, BBH, circular = TRUE), col = "blue")
  legend("topleft", legend = c("No noise", "Gaussian noise", "Impulsive noise"), col = c("red", "green", "blue"), lty = 1, cex = 0.6)
  
  # Butterworth filter (low freq)
  plot(t, x0, col = "red", type = "l", main = "Butterworth filter (low freq)", ylab = "Amplitude", xlab = "Time")
  lines(t, filter(x1, BBL, circular = TRUE), col = "green")
  lines(t, filter(x2, BBL, circular = TRUE), col = "blue")
  legend("topleft", legend = c("No noise", "Gaussian noise", "Impulsive noise"), col = c("red", "green", "blue"), lty = 1, cex = 0.6)
}
